...
3. On the **Basics** tab:
   ...
   - **Name**: use **your first name**. Everyone on the fund shares one subscription, so identifiable names keep the resources easy to manage; avoid vague names like `test1`. The name becomes your endpoint hostname, for example `https://<your-first-name>.openai.azure.com`, and it must be unique across Azure; append a few digits if your first choice is taken.
